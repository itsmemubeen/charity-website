...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            <h4 class="card-title">All Petitions List</h4>
                ...
                <div id="example23_wrapper" class="dataTables_wrapper container-fluid dt-bootstrap4"><div class="dt-buttons">          
                    ...
                    <thead>
                        <tr>
                            ...
                            <th rowspan="1" colspan="1">Petition Created By</th>
                            <th rowspan="1" colspan="1">Petition Title</th>
                            <th rowspan="1" colspan="1">Petition Desc</th>
                    ...
                    <tbody>
                        @php
                            $i = 1;
                        @endphp
                    @foreach ($petition as  $data)
                    <tr role="row" class="odd">
                            <td class="sorting_1">{{ $i }}</td>
                            <td>{{ $data->members->f_name }}</td>
                            <td>{{ $data->p_title }}</td>
                            <td>{{ substr($data->p_desc, 0,20) }}</td>
                            <td>{{ $data->created_at }}</td>
                            {{-- <td>{{ date_format($data->created_at,"d/m/Y") }}</td> --}}
                            <td>
                                <a href="{{ route('petition.edit',[$data->id]) }}"><button class="dt-button buttons-copy buttons-html5 btn btn-primary me-1" style="float: right">View</button></a>
                                <form method="POST" action="{{route('petition.destroy',[$data->id])}}">
                                    @csrf
                                    @method('delete')
